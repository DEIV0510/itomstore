...
/**
 * El corazon de la home: todos los productos disponibles, con filtro instantaneo
...
export default function ProductsGrid() {
  ...
  const chips = useMemo(() => {
    ...
    return [{ id: 'todos', name: 'Todos', icon: null }, ...withStock.map((c) => ({ id: c.id, name: c.name, icon: c.icon }))]
  }, [])
        ...
        <Reveal className="mb-8 flex flex-col gap-5 sm:mb-10 sm:flex-row sm:items-end sm:justify-between">
          <div className="max-w-xl">
            ...
            <p className="body-lg mt-3">
              {PRODUCTS.length} referencias disponibles. Confirmamos precio y disponibilidad por WhatsApp.
            </p>
          </div>
            ...
            {chips.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  {c.name}
                  <span className={`text-[11px] font-medium ${on ? 'text-gold-400' : 'text-silver-700'}`}>{count}</span>
                </button>
              )
            })}
          </div>
